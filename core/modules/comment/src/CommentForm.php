  ...
  public function validate(array $form, FormStateInterface $form_state) {
    ...
    if (!$entity->isNew()) {
      ...
      if ($form_state->getValue('name') && !$form_state->getValue('is_anonymous') && !$account) {
        $form_state->setErrorByName('name][0][value', $this->t('You have to specify a valid author.'));
      }
    }
    elseif ($form_state->getValue('is_anonymous')) {
      ...
      if ($form_state->getValue('name')) {
        $accounts = $this->entityManager->getStorage('user')->loadByProperties(array('name' => $form_state->getValue('name')[0]['value']));
        if (!empty($accounts)) {
          $form_state->setErrorByName('name][0][value', $this->t('The name you used belongs to a registered user.'));
        }
      }
    }
  }
